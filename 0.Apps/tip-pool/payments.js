...
let paymentTrs = document.querySelectorAll('#paymentTable tbody tr');
...
function appendPaymentTable(curPayment) {
  ...
  paymentTrs = document.querySelectorAll('#paymentTable tbody tr');
}
...
function removePaymentFromAllPayments(paymentKey){
  delete allPayments[paymentKey];
}

//Remove tr from DOM and the payment property from the allPayments object
paymentTbody.addEventListener('click', function(e){
  let childOfTargetToRemove = e.target
  let paymentKey = e.target.parentElement.id;

  if(childOfTargetToRemove.innerText === 'X'){
    removeFromTable(childOfTargetToRemove);
    removePaymentFromAllPayments(paymentKey);
  }
})